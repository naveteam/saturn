...
export const AutoAsync = () => {
  ...
  return <Autocomplete loading={loading} onChange={() => console.log('oi')} onChangeText={(e) => setTyped(e)} options={options} optionValue='country' optionLabel='name' label='teste' />
}
...
O component de Autocomplete sinaliza arquivos que passaram por um processo de upload ou estão prontos para passar por um processo de download.
