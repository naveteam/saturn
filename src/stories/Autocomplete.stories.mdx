  ...
  >
    {() => {
      ...
      useEffect(() => {
        setLoading(true)
        if(typed.length > 4)
        fetch(`http://universities.hipolabs.com/search?name=${typed}`)
          .then(res => res.json())
          .then(
            (universities) => {
              setLoading(false);
      ...
      return <ThemeProvider><Autocomplete minChar={4} loading={loading} onChange={() => console.log('oi')} onChangeText={(e) => setTyped(e)} options={options} getOptionValue={(e) => e.name} getOptionLabel={(e) => e.name} label='teste' /></ThemeProvider>
    }}
  </Story>
</Canvas>
