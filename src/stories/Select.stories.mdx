...
<Story
  name='Base'
  args={{ options: optionsSelect, placeholder: 'Selecione uma opção', caption: 'Caption' }}
  parameters={{
    design: {
      type: 'figma',
...
<Canvas>
  ...
  >
    <ThemeProvider>
      <Select name='disabled' label='Disabled' placeholder='Selecione uma opção' disabled caption='Disabled' />
    </ThemeProvider>
  </Story>
</Canvas>
